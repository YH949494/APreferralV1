...
def process_welcome_reminders(*, now_ref: datetime | None = None, batch_limit: int | None = None, db_ref=None, send_fn=None, bot_send_fn=None) -> dict:
    ...
    for doc in cursor:
        ...
        if completed == 1 and day1_at and not doc.get("reminder_20h_sent"):
            ...
            if elapsed >= timedelta(hours=20) and _welcome_adaptive_send_ready(uid, db_ref=db_ref, now_ts=now_ts, elapsed=elapsed, threshold_hours=20):
        ...
        if completed == 1 and day1_at and not doc.get("reminder_28h_sent"):
            elapsed = now_ts - day1_at
            if elapsed >= timedelta(hours=28) and _welcome_adaptive_send_ready(uid, db_ref=db_ref, now_ts=now_ts, elapsed=elapsed, threshold_hours=28):
        ...
        # never overtakes/duplicates reminders #1-3.
        if not doc.get("recovery_sent"):
            ...
            if recovery_anchor and (now_ts - recovery_anchor) >= timedelta(hours=WELCOME_RECOVERY_AFTER_HOURS):
                ...
                if ok:
                    ...
                    log_welcome_event("welcome_recovery_sent", uid, now=now_ts)
                    recovery_sent += 1
                else:
                    send_failed += 1
