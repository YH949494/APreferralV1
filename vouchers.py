...
def verify_telegram_init_data(init_data_raw: str):
    ...
    if not provided_hash:
        # Telegram now supports both `hash` and `signature`; accept either so we
        # don't reject valid init data generated by newer clients.
        provided_hash = parsed.get("signature", [""])[0]
