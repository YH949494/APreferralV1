...
(function () {
  ...
  //
  // The Mission operator experience lives in static/mission-admin.js, on its
  // own nav view, with explicit LIST / CREATE / VIEW / EDIT modes.
  //
  // It is a separate module on purpose. The previous implementation shared
  // THIS screen's "Create Campaign" form between "create a new mission" and
  // "edit the selected one", and every wrong-campaign write found in review
  // came from those two intents sharing one set of controls, one set of
  // hydrated values and one save handler. The Mission fields are gone from
  // the form below, so nothing here can write a mission, and the Mission
  // surface has no access to this form.

  function loadMissionPool() {
    var mod = window.MissionAdmin;
    if (!mod) { statePanel("mp-root", "error", "Mission admin module failed to load."); return; }
    mod.init({
      $: $, api: api, apiPost: apiPost, apiPostJson: apiPostJson, apiPutJson: apiPutJson,
      toast: toast, esc: esc,
  ...
  // Entry point from the Player Campaigns table: navigate to the Mission
  // surface and open that campaign's read-only operations view. It never
  // opens a create form for an existing campaign.
  function openMissionAdmin(campaignId) {
