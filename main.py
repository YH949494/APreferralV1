...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Welcome! Use /checkin to earn XP or /referral to invite others.")

async def checkin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    ensure_user(user.id, user.username)
    if not can_checkin(user.id):
        await update.message.reply_text("⏳ Come back after 24 hours to check in again.")
        return
    checkin_user(user.id)
    await update.message.reply_text("✅ Check-in successful! +20 XP")

async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    link = generate_referral_link(user.id, user.username)
    await update.message.reply_text(f"Invite your friends: {link}")
